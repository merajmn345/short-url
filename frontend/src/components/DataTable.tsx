...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    const copyClipboard = async (url: string) => {
        try {
            ...
            alert(`URL cpoied: ${serverUrl}/shortUrl/${url}`);
        } catch (error) {
            console.log(error);
        }
    };
        ...
        return data.map((item) => {
            return (
                <tr key={item._id} className="border-b text-white bg-gray-600 hover:bg-white hover:text-gray-800 ">
                    <td className="px-6 py-3 w-6/12 break-words">
                    ...
                    <td className="px-6 py-3">
                        <div className="flex content-center">
                            <div className="cursor-pointer px-2" onClick={() => copyClipboard(item.shortUrl)}>
                                <svg
        ...
        <div className="container mx-auto pt-2 pb-10">
            <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
                <table className="w-full table-fixed text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tbody className="px-6 py-3 w-6/12">{renderTableData()}</tbody>
                </table>
            </div>
        </div>
    );
